...
docker build . --tag msi-cad-vw-keycloak --file github/setup/keycloak.Dockerfile
docker tag msi-cad-vw-keycloak:latest europe-west1-docker.pkg.dev/$PROJECT_ID/docker-repo/keycloak:latest
docker push europe-west1-docker.pkg.dev/$PROJECT_ID/docker-repo/keycloak:latest
...
gcloud run deploy msi-cad-vw-keycloak --image=europe-west1-docker.pkg.dev/$PROJECT_ID/docker-repo/keycloak:latest \
    --platform=managed \
	--region=europe-west1 \
	--port=8080 \
	--memory=1Gi \
